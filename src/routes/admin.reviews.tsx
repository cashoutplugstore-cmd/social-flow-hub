...
function AdminReviews() {
  ...
  const moderate = async (id: string, next: Review["status"]) => {
    ...
    const { error } = await supabase.rpc("admin_moderate_review", {
      _review_id: id,
      _status: next,
      _response: responses[id]?.trim() || null,
    });
    setSaving(null);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {loading ? <Loader2 className="mx-auto animate-spin" /> : rows.length === 0 ? <div className="surface-card rounded-2xl p-10 text-center text-muted-foreground">لا توجد مراجعات في هذا القسم.</div> : <div className="space-y-4">{rows.map(r => <article key={r.id} className="surface-card rounded-2xl p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><div className="font-bold">{"★".repeat(r.rating)}{"☆".repeat(5 - r.rating)} {r.verified_purchase && <span className="ms-2 text-xs text-primary">شراء موثّق</span>}</div><div className="text-sm text-muted-foreground">{r.display_name || "عميل"} · {new Date(r.created_at).toLocaleString("ar")}</div></div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button size="sm" variant="hero" disabled={saving === r.id} onClick={() => void moderate(r.id, "approved")}><Check />نشر</Button>
          <Button size="sm" variant="outline" disabled={saving === r.id} onClick={() => void moderate(r.id, "hidden")}><EyeOff />إخفاء</Button>
          <Button size="sm" variant="outline" disabled={saving === r.id} onClick={() => void moderate(r.id, "flagged")}>تحديد كمشكلة</Button>
          <Button size="sm" variant="destructive" disabled={saving === r.id} onClick={() => void remove(r.id)}><Trash2 />حذف</Button>
        </div>
      </article>)}</div>}
    </div>
  </PageShell>;
