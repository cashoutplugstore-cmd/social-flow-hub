...
export const getCatalog = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [cats, svcs] = await Promise.all([
    supabase
      ...
      .select("id, slug, name_ar, name_en, description_ar, sort_order")
      .eq("is_active", true)
      .order("sort_order"),
    supabase
      .from("services")
      .select(
        "id, slug, name_ar, short_description_ar, description_ar, price_per_unit, unit_size, min_quantity, max_quantity, delivery_time_ar, input_label_ar, input_type, instructions_ar, notes_ar, popularity, is_active, category_id",
      )
      .eq("is_active", true)
  ...
  return {
    categories: (cats.data ?? []) as PublicCategory[],
    services: (svcs.data ?? []) as PublicService[],
...
export const getServiceBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      category: (category ?? null) as PublicCategory | null,
      related: (related ?? []) as PublicService[],
    };
  });
